...
ZLKJBSolver::ZLKJBSolver(Oink *oink, Game *game) : Solver(oink, game), Q(game->n_nodes), D(game->n_nodes)
{
}

ZLKJBSolver::~ZLKJBSolver()
{
    delete[] inverse;
}

int
ZLKJBSolver::attractExt(int i, int r, std::vector<int> *R)
{
        ...
        while (!Q.empty()) {
            ...
            for (int from = *_in; from != -1; from = *++_in) {
                ...
                if (owner[from] == pl) {
                    ...
                    // if(strategy[from] > -1 && strategy[from] != cur) continue;
                    // Double decrement is impossible since all nodes are attracted to heads before attractLosing starts.
                    // The only dependen nodes with bot region are freshly invalidated.
                    // All nodes mustr have BOT or region >= 0. Thus the negative counts are correct and will not be double decremented.
                    // owned by other parity
                    int count = region[from];
                    if (count == BOT) {
                        // compute count (to negative)
